  ...
  return (
    <Section3 className="py-[60px]">
      <div className="xl:flex justify-between mx-auto w-[80%] max-w-[1100px]">
        <div>
          <div className="h-fit font-bold text-[40px] leading-[48px]">
            {e.title}
            {e.closed && " (Closed)"}
          </div>
          {leading.value !== 0 && (
            <p className="h-fit font-normal text-[16px] leading-[24px] text-[#00000080]">
              {e.closed ? "Outcome" : "Leading"}: {leading.group} (with{" "}
              {leading.value} {leading.value === 1 ? "vote" : "votes"})
            </p>
          )}
          ...
          <div className="h-fit mt-[15px]">
            ...
            <ul className="list-disc list-inside">
              {data.map((d, i) => {
                return (
                  <li key={i}>
                    {d.group} ({d.value} {d.value === 1 ? "vote" : "votes"})
                    {userVoted !== -1 &&
                      candidates[e.vote[userVoted]] === d.group && (
                        <b>(You voted)</b>
                      )}
                  </li>
                );
